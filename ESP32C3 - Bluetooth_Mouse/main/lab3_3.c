...
#define HIDD_DEVICE_NAME            "Novello_ESP32C3"
...
void read_accel() {
	...
	if(sec_conn){
		if (data_x > 600){
			if(data_y > 600){
				esp_hidd_send_mouse_value(hid_conn_id, 0, -4, -4);
			} else if (data_y < -600){
				esp_hidd_send_mouse_value(hid_conn_id, 0, -4, 4);
			} else {
				esp_hidd_send_mouse_value(hid_conn_id, 0, -4, 0);
			}
		} else if (data_x < -600){
			if (data_y > 600){
				esp_hidd_send_mouse_value(hid_conn_id, 0, 4, -4);
			...
			esp_hidd_send_mouse_value(hid_conn_id, 0, 0, -4);
		} else if (data_y < -600) {
			esp_hidd_send_mouse_value(hid_conn_id, 0, 0, 4);
		}
	}
...
void hid_demo_task(void *pvParameters)
{
    ...
    while(1) {
	    ...
	    //esp_hidd_send_mouse_value(hid_conn_id, 0, -40, 0);
